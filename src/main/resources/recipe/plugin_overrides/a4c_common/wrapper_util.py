...
def handle_resource_ids():
  try:
    lock = Lock("/tmp/lock_name_{}.tmp".format(ctx.deployment.id))
    lock.acquire()

    # Get the last instance resource index
    instance_index = _get_index()
...
def _get_index():
  if(os.path.isfile(_get_filepath())):
    with open(_get_filepath(), 'r') as f:
      line = f.read()
      if(line):
        return int(line)
  return 0
...
def _write_index(index):
  ctx.logger.debug("[A4C_VOLUME] writing {} into {}".format(index, _get_filepath()))
  with open(_get_filepath(), 'w') as f:
    f.write(str(index))
