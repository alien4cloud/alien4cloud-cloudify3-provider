...
#
# https://github.com/cloudify-cosmo/cloudify-openstack-plugin/blob/1.3.1/openstack_plugin_common/__init__.py
#
# [a4c_override]: 
#  - When retrieving 'use_external_resource' and 'resource_id' values, search in the node instance runtime properties
#
...
def use_external_resource(ctx, sugared_client, openstack_type,
    ...
    #[a4c_override] remove the test with floating IP
    ctx.instance.runtime_properties[OPENSTACK_NAME_PROPERTY] = sugared_client.get_name_from_resource(resource)

    ctx.logger.info('Using external resource {0}: {1}'.format(
        openstack_type, ctx.instance.runtime_properties['resource_id']))
    return resource
...
def is_external_resource(ctx):
    # [a4c_override]
    return is_external_resource_by_properties(ctx.instance.runtime_properties)
